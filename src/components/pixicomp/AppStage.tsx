...
const AppStage = ({ payout, game_anim_status, dimension, pixiDimension, trigParachute }: { payout: number, game_anim_status: gameAnimStatusType, dimension: dimensionType, pixiDimension: dimensionType, trigParachute: { uniqId: number, isMe: boolean } }) => {
    ...
    const addParachute = (isMe: boolean) => {
        ...
        const pos = {
            ...
            y: dimension.height - 40 - (1 - pulse) * curveFunction(planeX, { width: dimension.width - 40, height: dimension.height - 40 })
        }
        if (isMe) {
    ...
    return (
        <Container>
                    ...
                    <Container>
                        ...
                        {parachutesMe.map((item, i) =>
                            <AnimatedSprite
                                ...
                                position={item}
                                loop={false}
                                onComplete={() => setParachutesMe(prev => prev.slice(1))}
                            />
                        )}
                        {parachutesElse.map((item, i) =>
                            <AnimatedSprite
                                ...
                                position={item}
                                loop={false}
                                onComplete={() => setParachutesElse(prev => prev.slice(1))}
                            />
                        )}
                    </Container>
                }
            ...
            <Container mask={dotRef.current}>
                {Array.from({ length: 30 }, (_, i) => (i * 140000 / (pixiDimension.width || 1) + 10)).map((coor, i) =>
                    <Container key={i}>
                        <Sprite
                            scale={0.4}
